...
function updateBooksFromStorage() {
  const loadedBooks = StorageService.loadFromStorage(BOOKS_KEY);
  if (loadedBooks) {
    books = [...loadedBooks];
  }
}
...
function reviewSubmit(review, bookId) {
  getBookIdxById(bookId).then(idx => {
    books[idx].reviews
    if (!books[idx].reviews) books[idx].reviews = [review];
    else books[idx].reviews.push(review);
...
function addBook(bookToAdd) {
  const bookIdx = books.findIndex(book => book.id === bookToAdd.id)
  if (bookIdx === -1) {
